...
var Navbar = function () {
    ...
    var _c = useState(false), isScrolled = _c[0], setIsScrolled = _c[1];
    var session = useSession().data;
    console.log(session);
    console.log(session === null || session === void 0 ? void 0 : session.user);
    var toggleModal = function () { return setShowModal(function (prev) { return !prev; }); };
    useEffect(function () {
    ...
    return (<div className={"fixed z-50 h-16 w-full top-0 left-0 ".concat(isScrolled ? "shadow-md backdrop-blur" : "", " ")}>
      <div className="h-full w-2/3 mx-auto flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2 transition-all">
          <h1 className={"".concat(isScrolled ? "text-blue-600" : "text-[#cec7c7]", " text-2xl font-bold ")}>
            Nirvana
          </h1>
          <AiOutlineHome size={25} color={"".concat(isScrolled ? "rgb(37 99 235)" : "#cec7c7", " ")}/>
        </Link>
        <div>
          <div className="cursor-pointer " onClick={toggleModal}>
            <AiOutlineUser size={30} color={"".concat(isScrolled ? "rgb(37 99 235)" : "#cec7c7")}/>
          </div>
          {showModal && (<>
