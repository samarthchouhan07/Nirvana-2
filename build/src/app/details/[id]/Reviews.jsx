...
const Reviews = ({ id }) => {
    ...
    const currentReviews = reviews === null || reviews === void 0 ? void 0 : reviews.slice(itemOffset, endOffset);
    ...
    return (<>
      <div className="mt-8 flex items-center gap-6">
        {Array.from(Array(5).keys()).map((_, number) => (<span key={number} onClick={() => setSelectedStar(number + 1)} className={`${selectedStar === number + 1 ? "scale-125" : ""}
      ...
      <div className="mt-8 flex items-center gap-28 border rounded-lg py-4 px-6 w-max">
        <input className="outline-none" type="text" placeholder="Leave your opinion..." onChange={(e) => setText(e.target.value)} value={text}/>
      ...
      <div className="mt-16 h-[900px] flex flex-col gap-24 w-1/3">
        {currentReviews === null || currentReviews === void 0 ? void 0 : currentReviews.map((review) => (<Review key={review.id} review={review}/>))}
